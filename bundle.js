...
function obtainLanguageCheese(thickness, chunkiness) {
	function recursiveCheese(name, depth, cheese) {
		if (depth > 0) languageCheeseChars.forEach(char => recursiveCheese(`${name}${char}`, depth - 1, cheese));
		else new Array(chunkiness).fill(null).forEach((_, index) => cheese[`${name}${index}`] = cheeseShred);
	}
...
async function main() {
	...
	if (cheesing) console.log("WARNING: Cheesing is enabled. Use --no-cheese to disable. This might take a while.");
	...
	await exec("zip -r ../pack.zip ./*", {cwd: "./build"});
	console.log("ok bundled @ pack.zip");
}
